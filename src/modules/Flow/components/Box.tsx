...
class Box extends Component<IProps, IState> {
    ...
    componentDidMount(){
        const content = this.state.ref?.current?.querySelector(".flow__boxes__item__content");
    ...
    render() {
        return (
            <Draggable onDrag={this.move} defaultPosition={{ x: this.props.item.x, y: this.props.item.y }}>
            <g ref={this.state.ref} className="flow__boxes__item">
              <rect className="flow__boxes__item__rect" width={this.state.width} height={this.state.height}></rect>
              <g className="flow__boxes__item__content" transform={`translate(${this.state.padding.x}, ${this.state.height / 2 - 4})`}>
